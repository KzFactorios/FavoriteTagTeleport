...
function SurfaceData:get_chart_tags(player)
    if not self.chart_tags or next(self.chart_tags) == nil then
        ...
        for _, tag in pairs(chart_tags) do
            local pos_string = helpers.map_position_to_pos_string({ x = tag.position[1], y = tag.position[2] }) -- Convert position format
            self.chart_tags[pos_string] = tag -- Assign using pos_string
        end
    end
